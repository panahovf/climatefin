...
# delete
del temp_directory, df_country_developing, df_country_emerging, df_country_unfccc, df_country_unfccc_dev, df_country_unfccc_dop, v_countrycodes_developing, v_countrycodes_emerging
...
for region in regions_with_extras:
    ...
    df.loc[df['fuel_type'] == "Coal", years_columns] = (
        df.loc[df['fuel_type'] == "Coal", years_columns]
        .div(df['Region'].map(df_power_coal_intensity), axis=0)
        .div((24 * 365))
        .div(df['Region'].map(df_power_coal_utilization), axis=0)
    )
...
for region in regions_with_extras:
    ...
    df.loc[df['fuel_type'] == "Gas", years_columns] = (
        df.loc[df['fuel_type'] == "Gas", years_columns]
        .div(df['Region'].map(df_power_gas_intensity), axis=0)
        .div((24 * 365))
        .div(df['Region'].map(df_power_gas_utilization), axis=0)
    )
...
for region in regions_with_extras:
    ...
    df.loc[df['fuel_type'] == "Oil", years_columns] = (
        df.loc[df['fuel_type'] == "Oil", years_columns]
        .div(df['Region'].map(df_power_oil_intensity), axis=0)
        .div((24 * 365))
        .div(df['Region'].map(df_power_oil_utilization), axis=0)
    )
...
# total capacity
# 1 - coal
for region in regions_with_extras + ["dopunfccc"]:
...
# Grouping for regional aggregated data
...
for region in regions_with_extras:
    globals()[f"df_byfuel_avoided_annual_{region.lower()}"] = globals()[f"df_totalcapacity_nz1550v2_{region.lower()}"].copy().assign(
        **globals()[f"df_totalcapacity_nz1550v2_{region.lower()}"][years_columns].diff(axis=1)
    )
